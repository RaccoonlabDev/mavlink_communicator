...
print_help() {
   echo "Wrapper under docker API for MAVLink communicator.
It encapsulates all necessary docker flags and properly handles image versions.
https://github.com/RaccoonlabDev/mavlink_communicator
...
setup_image_name_and_version() {
    TAG_NAME=v0.1.0
    DOCKERHUB_REPOSITOTY=ponomarevda/mavlink_communicator
